...
struct Client {
    ...
    std::string username;
    std::string joinTime;
    // На один сокет может писать поток клиента, broadcast и /test-поток —
    // отправка должна быть синхронизирована, иначе фреймы перемешаются.
...
// Отправка целого фрейма (header + payload) под per-client мьютексом.
...
static void broadcast(MessageType type, const std::string& payload, int senderSock) {
    // Снимем снапшот сокетов и мьютексов под clientsMutex,
    // чтобы не держать его пока send блокируется.
...
// ============================================================
// TEST: 10 сообщений с интервалом, в отдельном потоке
// ============================================================
        ...
        while (g_running) {
            ...
            switch ((MessageType)header.type) {
                ...
                case MessageType::LogRequest: {
                    ...
                    if (req == "TEST") {
                        // Заливка TEST в отдельном потоке, чтобы не блокировать
                        // чтение последующих команд от того же клиента.
